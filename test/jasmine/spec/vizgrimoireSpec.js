...
//                    }, "It took too long to load data", 300);               
//                runs(function() {
//                    $.each(Report.getDataSources(), function(index, DS) {
//                        if (DS.getName() === "mls")
...
//            });
    ...
    // empty: data empty ok so no viz ok
    function checkVizReport(report, empty) {
        if (!empty) empty = 0;
        if ($.inArray(report,['repos','companies','countries'])===-1) 
            return;
        var total_canvas = 0;
        ...
        var new_ncanvas = document.getElementsByClassName
            ('flotr-canvas').length;
        expect(new_ncanvas-ncanvas).toEqual(total_canvas-(2*empty));        
    }
    ...
    describe("Companies checking", function() {
        ...
        it("Companies basic viz should work", function() {
            // gmail company has closed, closers empty
            checkVizReport("companies",1);
        });
    });
