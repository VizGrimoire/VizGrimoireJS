...
evol_info_data <- function() {
	...
	data6 <- query(q)
    q <- paste("select count(distinct(s.id))/timestampdiff(month,min(s.date),max(s.date)) as avg_commits_month from scmlog s")
    data7 <- query(q)
    q <- paste("select count(distinct(a.file_id))/timestampdiff(month,min(s.date),max(s.date)) as avg_files_month from scmlog s, actions a where a.commit_id=s.id")
    data8 <- query(q)
    # q <- paste("select count(distinct(s.id))/count(distinct(p.id)) as avg_commits_author from scmlog s, people p where p.id=s.author_id")
	q <- paste("select count(s.id)/count(distinct(upp.udev)) from scmlog s, people p, upeople_people upp where s.author_id = p.id and p.id = upp.people_id")
    data9 <- query(q)
    # q <- paste("select count(distinct(s.author_id))/timestampdiff(month,min(s.date),max(s.date)) as avg_authors_month from scmlog s")
	q <- paste("SELECT count(distinct(upp.udev))/timestampdiff(month,min(s.date),max(s.date)) 
				AS avg_authors_month from scmlog s, upeople_people upp 
				WHERE s.author_id = upp.people_id")
    data10 <- query(q)
    q <- paste("select count(distinct(upp.udev))/timestampdiff(month,min(s.date),max(s.date)) 
				as avg_committers_month from scmlog s, upeople_people upp
				WHERE s.committer_id = upp.people_id")
    data11 <- query(q)
    q <- paste("select count(distinct(a.file_id))/count(distinct(upp.udev)) 
				as avg_files_author from scmlog s, actions a,  upeople_people upp
				where a.commit_id=s.id and s.author_id = upp.people_id")
    data12 <- query(q)
