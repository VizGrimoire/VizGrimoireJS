...
evol_info_data <- function() {
	...
	data6 <- query(q)
        ...
        data10 <- query(q)
        q <- paste("select count(distinct(s.committer_id))/timestampdiff(month,min(s.date),max(s.date)) as avg_committers_month from scmlog s")
        data11 <- query(q)
        q <- paste("select count(distinct(a.file_id))/count(distinct(s.author_id)) as avg_files_author from scmlog s, actions a where a.commit_id=s.id")
        data12 <- query(q)
